...
protocol ResponseGenerator {
    ...
    func generateResponse() async throws -> Response
}

extension ResponseGenerator {
    fileprivate func makeResponseData(with choices: [[String: Any?]])
        -> [String: Any]
    {
        return [
            "id": "gen-\(UUID().uuidString)",
            "object": self.session.toStream
...
func createResponseGenerator(from session: ResponseSession)
    -> any ResponseGenerator
{
    if session.toStream {
...
class StandardResponseGenerator: ResponseGenerator {
    ...
    func generateResponse() async throws -> Response {
        ...
        let response = Response(
            status: .ok,
            body: Response.Body(string: try jsonify(responseData))
        )
    ...
    private func makeNonChatChoice(from sessionResponse: SessionResponse)
        -> [String: Any?]
    {
        return [
    ...
    private func makeChatChoice(from sessionResponse: SessionResponse)
        -> [String: Any?]
    {
        return [
            "finish_reason": sessionResponse.finishReason?.rawValue,
    ...
    private func makeChoice(from sessionResponse: SessionResponse) -> [String:
        Any?]
    {
        ...
        response.body = .init(asyncStream: { writer in
            for await streamResponse in stream {
                ...
                try await writer.write(
                    .buffer(
                        ByteBuffer(
                            string: "data: \(try jsonify(responseData))\n\n"
                        )
                    )
                )
            }
            ...
            try await writer.write(
                .buffer(ByteBuffer(string: "data: [DONE]\n\n"))
            )

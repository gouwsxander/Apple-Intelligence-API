...
class ResponseSession {
    ...
    func getResponse() async -> SessionResponse {
        ...
        do {
            if let format = responseFormat, format.type == "json_schema", let schema = format.json_schema {
                if let rootProperty = schema.schema {
                    // Dynamic schema
                    let dynamicSchema = convertToDynamicSchema(name: schema.name, property: rootProperty)
                    let generationSchema = try GenerationSchema(root: dynamicSchema, dependencies: [])
                    let result = try await self.session.respond(to: self.prompt, schema: generationSchema, options: self.options)
                    
                    // GeneratedContent doesn't conform to Encodable easily, so we build a dictionary
                    let dict = try convertGeneratedContentToAny(result.content, schema: rootProperty)
                    let data = try JSONSerialization.data(withJSONObject: dict)
                    response.content = String(data: data, encoding: .utf8)
                } else {
                    response.content = "Error: No schema provided in json_schema."
    ...
    func streamResponses() -> sending AsyncStream<SessionResponse> {
        ...
        return AsyncStream { continuation in
            Task { @Sendable in
                do {
                    if let format = responseFormat, format.type == "json_schema", let schema = format.json_schema, let rootProperty = schema.schema {
                        // Snapshot streaming for dynamic schema
                        let dynamicSchema = convertToDynamicSchema(name: schema.name, property: rootProperty)
                        let generationSchema = try GenerationSchema(root: dynamicSchema, dependencies: [])
                        let stream = session.streamResponse(to: prompt, schema: generationSchema, options: options)
                        
                        for try await snapshot in stream {
                            var response = SessionResponse()
                            // Snapshot for dynamic schema is also GeneratedContent (or similar)
                            let dict = try convertGeneratedContentToAny(snapshot.content, schema: rootProperty)
                            let data = try JSONSerialization.data(withJSONObject: dict)
                            response.content = String(data: data, encoding: .utf8)
...
private func convertToDynamicSchema(name: String, property: JSONSchemaProperty) -> DynamicGenerationSchema {
    switch property {
    case .string(_, _):
...
private func convertGeneratedContentToAny(_ content: GeneratedContent, schema: JSONSchemaProperty) throws -> Any {
    switch schema {
    case .object(_, let properties, _):
        ...
        for (key, propertySchema) in properties {
            if let value = try? content.value(String.self, forProperty: key) {
                ...
                dict[key] = value
            } else if let value = try? content.value(Double.self, forProperty: key) {
                dict[key] = value
            } else if let value = try? content.value(Bool.self, forProperty: key) {
                dict[key] = value
            } else if let value = try? content.value(GeneratedContent.self, forProperty: key) {
                dict[key] = try convertGeneratedContentToAny(value, schema: propertySchema)
            } else if let value = try? content.value([GeneratedContent].self, forProperty: key) {
                if case .array(_, let itemSchema) = propertySchema {
                    dict[key] = try value.map { try convertGeneratedContentToAny($0, schema: itemSchema) }
                } else {
                    dict[key] = value.map { "\($0)" }
                }
            } else if let value = try? content.value([String].self, forProperty: key) {
                dict[key] = value
            } else if let value = try? content.value([Int].self, forProperty: key) {
                dict[key] = value
            } else if let value = try? content.value([Double].self, forProperty: key) {
    ...
    case .array(_, let itemSchema):
        ...
        if let value = try? content.value([String].self) {
            ...
            return try value.map { try convertGeneratedContentToAny($0, schema: itemSchema) }
        }
        return []
...
private func handleError(_ error: any Error, _ response: inout SessionResponse) {
    if let genError = error as? LanguageModelSession.GenerationError {
        ...
        response.finishReason = .error
        response.content = "Error: \(error.localizedDescription) (\(type(of: error)))"
    }
}
